...
function MeteorField({ trigger = 0 }: { trigger?: number }) {
  ...
  useEffect(() => {
    ...
    let w = 0, h = 0, raf = 0, time = 0, pulse = 0, target = trigger ? 1 : 0, px = .5, py = .5
    let burstStarted = trigger ? performance.now() : 0
    ...
    const run = () => {
      if (trigger && trigger !== lastTrigger) { lastTrigger = trigger; burstStarted = performance.now(); target = 1 }
...
function MorphingScene({ id, eyebrow, title, body, formation, children }: { id: string; eyebrow: string; title: string; body: string; formation: 'bits' | 'orbit' | 'matrix'; children: ReactNode }) {
  return <section id={id} className="relative min-h-[190vh] overflow-hidden border-b border-slate-200/80 dark:border-slate-800/80"><div className="sticky top-20 min-h-[calc(100vh-80px)]"><div className="absolute inset-0 bg-[#07111f]"><SteganographyField formation={formation} density={360} className="opacity-90" />{children}</div><div className="relative z-10 mx-auto flex min-h-[calc(100vh-80px)] w-full max-w-7xl items-center px-6 py-20 lg:px-10"><div className="max-w-2xl"><div className="font-mono text-[10px] uppercase tracking-[.24em] text-blue-300">{eyebrow}</div><h2 className="mt-6 max-w-xl text-5xl font-semibold tracking-[-.07em] text-white sm:text-7xl">{title}</h2><p className="mt-6 max-w-md text-base leading-8 text-blue-100/75">{body}</p></div></div></div></section>
}
